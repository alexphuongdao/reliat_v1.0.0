...
/**
 * Design notes. Ported 1:1 from frontend/screens/notes.jsx.
 * Mostly static content; the only interactivity is the section nav.
 */
import { type CSSProperties, useState } from "react";
import { Icon } from "../ui";

type SectionId =
  | "rationale" | "tokens" | "ia" | "specs"
  | "inventory" | "signature" | "keyboard"
  | "a11y" | "open" | "scope";
...
export function NotesScreen() {
  const [section, setSection] = useState<SectionId>("rationale");
  return (
    <div
      style={{
        display: "grid",
...
const noteStyles: Record<string, CSSProperties> = {
  ...
  p: { margin: "0 0 12px", textWrap: "pretty" as CSSProperties["textWrap"] },
  code: {
...
function TokenSwatch({ tokens, grid }: { tokens: TokenItem[]; grid?: boolean }) {
  return (
    <div
      style={{
        display: "grid",
    ...
    >
      {tokens.map((t) => (
        <div
          ...
          style={{
            display: "grid",
            gridTemplateColumns: "28px 180px 130px 1fr",
...
function Tokens() {
  return (
    <div>
      ...
      <p style={noteStyles.lede}>
        Concrete values. Live in <span style={noteStyles.inline}>tokens.css</span> in this prototype; should be a JSON or CSS package for engineering.
      </p>
        ...
        <div key={g.name} style={{ marginBottom: 24 }}>
          ...
          <div className="panel">
            <div
              style={{
                display: "grid",
                gridTemplateColumns: "180px 1.4fr 1.1fr 1.2fr",
...
function Keyboard() {
  const rows: Array<{ k: string; d: string; g: string }> = [
...
function A11y() {
  const items: Array<{ c: string; n: string }> = [
    ...
    <div>
      ...
      <div className="panel">
        {items.map((it, i) => (
          ...
          >
            <span style={{ color: "var(--ch-4)", marginTop: 2 }}>
...
function OpenQuestions() {
  const qs: Array<{ q: string; ctx: string }> = [
    { q: "How do we represent an outlier that spans multiple channels?", ctx: "Material transitions on a stockpile reclaim almost always show up on 2–3 channels in sequence. Right now an outlier has one channelId. Two options: (1) compound outliers with multiple channelIds and a primary; (2) link outliers by \"event\" — sibling pointers. I lean (2) because it preserves channel-level triage. Need 30 min with engineering." },
    { q: 'What is the threshold for "critical"?', ctx: "Brief says saturated but not fluorescent. I picked 3σ as the implicit cutoff in the data, but the customer probably wants this configurable per channel per metric. Where does it live in the UI — Library > Channels, or a global \"thresholds\" surface?" },
    { q: "Should the agent drawer be summonable from outside the app?", ctx: "A natural progression is a tray-icon / Slack-style entry point that opens the agent with an outlier link from an email or pager. Out of v1.0.0, but it affects the URL shape we adopt now." },
    { q: "Channel count > 16: virtualize the vitals rail, or introduce groups?", ctx: 'I picked 12 as the comfortable visible upper bound. Larger mines will exceed this. "Saved channel groups" is more useful long-term than virtualization — the rail is supposed to be glanceable. Defer the call until we have a 24-channel customer to ask.' },
    { q: "When the agent is wrong, where does feedback go?", ctx: 'Brief is silent. Suggestion: a per-turn "wrong" / "missed something" affordance that opens a tiny form, attached to the conversation. We need to specify what we do with it (eval set? thumbs to a router model?). Affects whether we ship the button at v1.0.0 or wait.' },
    { q: 'Is "shift handoff" a screen?', ctx: 'The shift summary on Pulse is one paragraph. I think there is a 1.1 surface called "Handoff" that produces a longer artifact — generated automatically at shift change, optionally exported as PDF. Out of v1.0.0 but worth designing the model now so the Pulse summary fits cleanly into it.' },
    { q: "CSV ingest UX for 2GB files", ctx: "The Uploads tile is light. A 2GB ingest is a multi-minute process. We need a real progress + parse-preview + rejected-rows triage view. That is its own design pass; the brief intentionally underweights Library, and I followed that. Flag for 1.1." },
    { q: "Light mode — when?", ctx: 'I designed the dark mode well. Light is "considered but secondary." I would defer it past 1.0 unless customers ask. A bad light mode would hurt the product more than no light mode.' },
  ];
  return (
    <div>
          ...
          <div key={i} className="panel" style={{ padding: "14px 16px" }}>
            ...
            <p
              style={{
                ...
                lineHeight: 1.6, textWrap: "pretty" as CSSProperties["textWrap"],
              }}
            >
              {it.ctx}
            </p>
